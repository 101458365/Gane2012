...
int main(void)
{
    ...
    stbi_uc* pixelsHead = stbi_load("./assets/textures/ct4_grey.png", &texHeadWidth, &texHeadHeight, &texHeadChannels, 0);
    ...
    Vector3 camPos{ 0.0f, 7.0f, 17.0f };
    Vector3 camLook{ V3_FORWARD };
    Quaternion camRotation = QuaternionIdentity();
    ...
    Mesh headMesh, cubeMesh, sphereMesh, ct4Mesh, planeMesh;
    ...
    Vector3 floorColor = { 0.5f, 0.5f, 0.5f };
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        cameraPitch += mouseDelta.y * mouseScale;
        cameraYaw += mouseDelta.x * mouseScale;
       
        camRotation = FromEuler(-cameraPitch * DEG2RAD, -cameraYaw * DEG2RAD, 0.0f);

        if (IsKeyDown(GLFW_KEY_W))
        {
            camPos.z -= objectDelta;
            ;
        }
        ...
        Matrix view = LookAt(camPos, camPos - Rotate(camLook, camRotation), V3_UP);
        ...
        case 3:
            ...
            glUseProgram(shaderProgram);
            world = objectMatrix * Scale(100.0f, 100.0f, 10.0f) * RotateX(90 * DEG2RAD) * Translate(-50.0f, 0.0f, -50.0f);
            mvp = world * view * proj;
            u_mvp = glGetUniformLocation(shaderProgram, "u_mvp");
            u_color = glGetUniformLocation(shaderProgram, "u_color");
            glUniformMatrix4fv(u_mvp, 1, GL_FALSE, ToFloat16(mvp).v);
            glUniform3fv(u_color, 1, &floorColor.x);
            DrawMesh(planeMesh);
